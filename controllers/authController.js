...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  const correctPwd = await user.correctPassword(req.body.currentPassword, user.password);
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!freshUser) {
    return next(
      new AppError(
        "The user this token belongs to is no longer exist. Kindly register for a new account",
        401
      )
    );
  }
